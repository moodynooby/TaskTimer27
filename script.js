...
function exportTasksToCSV() {
    let csvContent = 'data:text/csv;charset=utf-8,Task Name,Description,Time Spent (seconds)\n';
    tasks.forEach(task => {
        csvContent += `${task.name},${task.description},${task.timeSpent}\n`;
    });
...
function importTasksFromCSV(event) {
    ...
    reader.onload = function(e) {
        ...
        rows.forEach(row => {
            ...
            const [name, description, timeSpent] = columns.map(col => col.trim());
            if (name) {
                const task = {
...
exportBtn.className = 'btn btn-primary btn-outline';
exportBtn.addEventListener('click', exportTasksToCSV);

const importInput = document.createElement('input');
importInput.type = 'file';
importInput.accept = '.csv';
importInput.className = 'btn btn-secondary btn-outline';
importInput.addEventListener('change', importTasksFromCSV);

// Add buttons to the DOM
const controlsContainer = document.getElementById('taskControls'); // Add this container in your HTML if it doesn't exist
